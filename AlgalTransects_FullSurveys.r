...
# subset data for full surveys ####
  ...
  as.Date('2013-07-21'))
	...
	for (i in 1:length(FullSurveyDates)){
		x = AlgalTransects2[which(as.Date(AlgalTransects2$Rdate) %in% FullSurveyDates[[i]]),]
		x$FullSurvey = FullSurveyDates[[i]][1]
		AlgalTransectsFS = rbind(AlgalTransectsFS,x)
	}

# write file
  # use file name: 'AlgalTransects_FullSurveys.txt'
write.table(AlgalTransectsFS, file.choose(), sep = '\t', col.names = T, row.names=F, quote=F)
...
# write data file
  # use file name: 'AlgalTransects_FullSurveysMeanHeight.txt'
write.table(atFSwet_sMean, file.choose(), sep = '\t', col.names = T, row.names=F, quote=F)
...
# write data file
  # use file name: 'AlgalTransects_FullSurveysPeakBloom.txt'
write.table(atFSwet_sMax, file.choose(), sep = '\t', col.names = T, row.names=F, quote=F)
