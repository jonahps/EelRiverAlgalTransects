# This script will calculate the average summer light irradiance along Mary's 4 transects from April 15 - June 21
...
# Extract the weeks of the growing season April 15 - June 21 (approximately weeks 15 - 27)
	sml <- sm[which(sm$week >=15 & sm$week <=26),]
...
# Estimate the xstrm watt values for xstream integers not measured by the model
	...
	# Write a csv for wattavgf
	  ...
	  #setwd("/Users/keithgregson/Google Drive/Algal Timeseries/Processed data files/Light Model")
		#write.csv(wattavgf, file="LightModel_MarysTransects_SummerAvg.csv", row.names=FALSE)
